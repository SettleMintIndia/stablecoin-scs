...
interface IZKPVerifier {
    ...
    function getZKPRequests(uint256 startIndex, uint256 length) external view returns (ZKPRequest[] memory);

    function isProofVerified(address sender, uint64 requestId) external view returns (bool);

    function getProofStatus(address sender, uint64 requestId) external view returns (ProofStatus memory);

    function getProofStorageField(address user, uint64 requestId, string memory key) external view returns (uint256);
}
